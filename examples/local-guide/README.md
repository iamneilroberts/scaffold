...
# Search for nearby places (auth required — use the ADMIN_KEY from wrangler.toml)
curl -X POST http://localhost:8787 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer change-me-in-production" \
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"guide:search_nearby","arguments":{"lat":40.7128,"lng":-74.006}}}'
```

...
// actionable class ('managed' — an approved, in-house-vetted vendor), not on funnel
// state alone: a referral/other-source item can also reach 'confirmed' (it just caps
// there too, per the Gate's state cap for non-'managed' actionables), so state by
// itself doesn't prove locality. Gating on the class (rather than a single vendor's
// source string) also generalizes correctly to multiple approved local vendors.
export function renderGuestMenuLine(item: Item, caseState: Case): string {
  ...
  const isApprovedLocalSource = item.stamp.actionable === 'managed';
